...
class AttackProfile:
    ...
    def __init__(self, noise_level: float = 1.0, **kw):
        self.t0 = time.time()
        self.noise_level = noise_level  # 1.0 = dégradation normale, 2.0 = forte
...
class DroneController:
    ...
    def _set_guided_mode(self):
        # ArduRover : GUIDED = mode 15  (ArduCopter : GUIDED = mode 4)
        GUIDED = 15
        hb = self.mav.recv_match(type="HEARTBEAT", blocking=True, timeout=2.0)
        if hb and hb.custom_mode != GUIDED:
            print(f"\n  {_Y}[CTRL] Passage en mode GUIDED (15)…{_X}", end=" ", flush=True)
    ...
    def _send(self, spoof: Dict) -> bool:
        TYPE_MASK_POS_VEL = (
            0b0000_1111_1000  # ignorer acc, yaw, yaw_rate
        )
        try:
...
def _parse_gps(msg) -> Optional[Dict]:
    try:
        return {
            ...
            "h_acc_sim": 2.0,  # valeur de base simulée en mètres
        }
    except Exception:
        return None
...
def main():
    p = argparse.ArgumentParser(
        description="GPS Spoofing Simulator — ArduCopter SITL",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
  python attaque.py --mode-both --attack drift --drift-rate 1.5
        """
    )
    ...
    mg.add_argument("--mode-both",  action="store_true", help="Drone + proxy")
    ...
    p.add_argument("--noise-level",       type=float, default=1.0,
                   help="Intensité de dégradation du signal (défaut: 1.0, fort: 2.0)")
    # Profils
    ...
    print("\n" + "=" * 70)
    print("  GPS SPOOFING SIMULATOR v2 — ArduCopter SITL")
    print("=" * 70)
    print(f"  Mode           : {sim_mode.upper()}")
    print(f"  Attaque        : {_R}{_B}{args.attack.upper()}{_X}")
    print(f"  Noise level    : {args.noise_level:.1f}x")
    print(f"  Contrôle SITL  : {'✓ SET_POSITION_TARGET_GLOBAL_INT' if do_control else '✗'}")
    print(f"  Proxy détecteur: {'✓ → port ' + str(args.forward_port) if do_proxy else '✗'}")
    print("=" * 70)

    if do_control:
        print(f"\n  {_Y}PRÉREQUIS : Drone armé + en vol + mode GUIDED dans QGC !{_X}")
        print(f"  QGC → Arm → Takeoff → Flight Mode: Guided\n")

    # Profil d'attaque
    degrade = not args.no_degrade
    kw = {"noise_level": args.noise_level}
    if args.attack == "drift":
        kw.update({"drift_rate_m_s": args.drift_rate,
                   "direction_deg": args.direction, "degrade": degrade})
    ...
    print(f"  Connexion: {args.connection} …", end=" ", flush=True)
    ...
    if hb:
        mode  = COPTER_MODES.get(hb.custom_mode, str(hb.custom_mode))
        armed = bool(hb.base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED)
        print(f"  Mode: {mode}  |  Armé: {'OUI ✓' if armed else 'NON ✗'}")
    ...
    if do_control:
        ...
        print(f"  Contrôleur démarré @ {args.control_rate} Hz")
        ...
        while True:
            ...
            msg = mav.recv_match(
                type=["GPS_RAW_INT", "GLOBAL_POSITION_INT",
                      "EKF_STATUS_REPORT", "HEARTBEAT", "SYS_STATUS"],
            ...
            if pre:
                ...
                if not profile_armed:
                    ...
                    if do_control:
                        print(f"  {_G}SET_POSITION_TARGET_GLOBAL_INT → drone navigue vers position spoofée{_X}")
                    if sim_mode == "proxy":
                        print(f"  {_Y}Mode proxy : drone immobile → vel_consistency détectable par le modèle{_X}")
                    print()
            ...
            if pre:
                ...
                print(f"\r[{ts}] {_G}PRE-ATTACK{_X}  "
                      ...
                      f"alt={real_gps.get('rel_alt', 0):.1f}m  sats={real_gps['sats']}  "
                      f"hdop={real_gps['hdop']:.2f}  "
                ...
                # Afficher la dégradation du signal pour debug
